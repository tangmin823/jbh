...
<div id="comments" class="live-comment dark-div">
	...
	$arrFixCommentPlaceholder = array(
								'comment_field' => '<input id="comment" name="comment" aria-required="true" placeholder="'.esc_html__('Your comment  *','17jbh').'">',
								'title_reply'       => esc_html__( 'YOUR COMMENTS', '17jbh' ),
								'title_reply_to'    => esc_html__( 'Leave a Reply to %s', '17jbh' ),
								'label_submit'    => esc_html__( 'SEND', '17jbh' ),
								);
	...
	<?php if ( have_comments() ) : ?>
		<h2 class="comments-title">
			<?php
				echo esc_html__('Comment', '17jbh').' (<span id="tdt-f-number-calc">'.number_format_i18n( get_comments_number() ).'</span>)';
			?>
		</h2>
        ...
        <input type="hidden" name="videopro_text_plst" value="<?php esc_html_e('Please type a comment.','17jbh'); ?>">
        <input type="hidden" name="videopro_text_dlc" value="<?php esc_html_e('Duplicate comment detected; it looks as though you\'ve already said that!','17jbh'); ?>">
        <input type="hidden" name="videopro_text_tfy" value="<?php esc_html_e('Thanks for your comment','17jbh'); ?>">
        <input type="hidden" name="videopro_text_plwa" value="<?php esc_html_e('Please wait a while before posting your next comment','17jbh'); ?>">
		<?php 
		$arr_all = array(
			'comment__not_in' => '',
			'post_id' => $post->ID,
	...
	?>
		<p class="no-comments"><?php esc_html_e( 'Comments are closed.', '17jbh' ); ?></p>
	<?php endif; ?>
	</div>
    </div>
</div><!-- #comments -->
