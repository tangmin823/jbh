...
class videopro_Popular_posts extends WP_Widget
{
	function __construct()
	{
		$options = array(
			...
			'description' 	=> esc_html__('Show popular posts', '17jbh')
			);
		parent::__construct('popular_posts_id', esc_html__('VideoPro - Popular Posts', '17jbh'), $options);
	}

	function form($instance)
	{
		$default_value 		= array(
			'title' 		=> esc_html__('Popular Posts', '17jbh'),
			'category' 				=> '',
			'style' 				=> '1',
			'tags' 					=> '',
			'post_ids' 				=> '',
		...
		$html 	= '';
		$html  .= '<p>';
		$html  .= '<label>' . esc_html__('Title', '17jbh') . ': </label>';
		...
		$html  .= '<label>' . esc_html__('Style', '17jbh') . ': </label>';
		$html  .= '<p>';
		$html  .= '<select name="' . $this->get_field_name('style') . '">
						<option value="1"' . $style1 . '>' . esc_html__('Style 1 - Vertical List, Small Thumbnail', '17jbh') . '</option>
						<option value="2"' . $style2 . '>' . esc_html__('Style 2 - Vertical List, Big Thumbnail ', '17jbh') . '</option>
						<option value="3"' . $style3 . '>' . esc_html__('Style 3 - Carousel', '17jbh') . '</option>
					</select>';
		$html  .= '</p>';

		$html  .= '<p>';
		$html  .= '<label>' . esc_html__('Category (Category ID or Slug)', '17jbh') . ': </label>';
		$html  .= '<input class="widefat" type="text" name="' . $this->get_field_name('category') . '" value="' . $category . '"/>';
		$html  .= '</p>';

		$html  .= '<p>';
		$html  .= '<label>' . esc_html__('Tags', '17jbh') . ': </label>';
		$html  .= '<input class="widefat" type="text" name="' . $this->get_field_name('tags') . '" value="' . $tags . '"/>';
		$html  .= '</p>';

		$html  .= '<p>';
		$html  .= '<label>' . esc_html__('Post IDs: (If this param is used, other params are ignored)', '17jbh') . ': </label>';
		$html  .= '<input class="widefat" type="text" name="' . $this->get_field_name('post_ids') . '" value="' . $post_ids . '"/>';
		$html  .= '</p>';

		$html  .= '<p>';
		$html  .= '<label>' . esc_html__('Number of items', '17jbh') . ': </label>';
		...
		$html  .= '<label>' . esc_html__('Number of days', '17jbh') . ': </label>';
		$html  .= '<p>';
		$html  .= '<select name="' . $this->get_field_name('number_of_days') . '">
						<option value="day"' . $one_day . '>' . esc_html__('1 day', '17jbh') . '</option>
						<option value="week"' . $one_week . '>' . esc_html__('1 week', '17jbh') . '</option>
						<option value="month"' . $one_month . '>' . esc_html__('1 month', '17jbh') . '</option>
						<option value="year"' . $one_year . '>' . esc_html__('1 year', '17jbh') . '</option>
					</select>';
		$html  .= '</p>';
		...
		$html  .= '<p><label>' . esc_html__('Order by', '17jbh') . ': </label></p>';
		$html  .= '<p>';
		$html  .= '<select name="' . $this->get_field_name('order_by') . '">
						<option value="latest"' . $latest . '>' . esc_html__('Latest', '17jbh') . '</option>
						<option value="most_viewed"' . $most_viewed . '>' . esc_html__('Most viewed', '17jbh') . '</option>
						<option value="most_liked"' . $most_liked . '>' . esc_html__('Most liked', '17jbh') . '</option>
						<option value="most_commented"' . $most_commented . '>' . esc_html__('Most commented', '17jbh') . '</option>
					</select>';
		$html  .= '</p>';
	...
	function widget($args, $instance)
	{
		...
		$number_of_headlines 	= isset($instance['number_of_headlines']) ? intval($instance['number_of_headlines']) : 5;
										...
										<div class="entry-content">                                        
											...
											<div class="picture">
												<div class="picture-content">
													<a title="' . get_the_title() . '" class="post-link" target="'. apply_filters('videopro_loop_item_url_target', '_self', $id) . '" href="' . esc_url($link_post) . '">
