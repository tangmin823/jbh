...
class Videopro_Widget_Categories extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__( 'A list of categories.','17jbh' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'videopro_categories', esc_html__( 'VideoPro - Categories','17jbh' ), $widget_ops );
	}
...
<!--                            --><?php //} ?>
	...
	public function form( $instance ) {
		...
		$layout = isset($instance['layout']) ? esc_attr($instance['layout']) : '';
		?>
		<p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e( 'Title:','17jbh' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>
		<input type="checkbox" class="checkbox" id="<?php echo esc_attr($this->get_field_id('count')); ?>" name="<?php echo esc_attr($this->get_field_name('count')); ?>"<?php checked( $count ); ?> />
		<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e( 'Show post counts','17jbh' ); ?></label><br />

		<input type="checkbox" class="checkbox" id="<?php echo esc_attr($this->get_field_id('hierarchical')); ?>" name="<?php echo esc_attr($this->get_field_name('hierarchical')); ?>"<?php checked( $hierarchical ); ?> />
		<label for="<?php echo esc_attr($this->get_field_id('hierarchical')); ?>"><?php esc_html_e( 'Show all sub-categories','17jbh' ); ?></label></p>
        
        <p>
            <label for="<?php echo esc_attr($this->get_field_id("layout")); ?>">
				<?php esc_html_e('Layout','17jbh');	 ?>:
                <span style="font-style: italic; display:block;"><?php esc_html_e('choose widget layout','17jbh');?></span>
                <select id="<?php echo esc_attr($this->get_field_id("layout")); ?>" name="<?php echo esc_attr($this->get_field_name("layout")); ?>">
                    <option value="icon"<?php selected( $layout, "icon" ); ?>><?php esc_html_e('Show category icon','17jbh');?></option>
                    <option value="thumb"<?php selected( $layout, "thumb" ); ?>><?php esc_html_e('Show category thumbnail','17jbh');?></option>
                </select>
            </label>
        </p>
		<?php
