...
function videopro_buddypress_widgets_init(){
    ...
    register_sidebar( array(
        'name' => __( 'BuddyPress Sidebar', '17jbh' ),
        'id' => 'bp_sidebar',
        'description' => __( 'Sidebar in BuddyPress Page.', '17jbh' ),
...
function videpro_buddypress_theme_options($settings){
    $settings['sections'][] = array(
                'id' => 'buddypress',
                'title' => esc_html__('BuddyPress', '17jbh')
            );
    
    $settings['settings'][] = array(
            'id'          => 'bp_sidebar',
            'label'       => esc_html__('Main Sidebar', '17jbh'),
            'desc'        => esc_html__('This Sidebar appears in all BuddyPress pages', '17jbh' ),
            'std'         => 'full',
            'type'        => 'select',
            'section'     => 'buddypress',
            'choices'     => array(
    ...
    $settings['settings'][] = array(
            ...
            'desc'        => esc_html__('This Layout setting applies for all BuddyPress pages', '17jbh' ),
            'std'         => 'fullwidth',
            'type'        => 'select',
            'type'        => 'radio-image',
            'section'     => 'buddypress',
    ...
    $settings['settings'][] = array(
            ...
            'label'       => esc_html__('Videos Tab Position In Member Profile', '17jbh'),
            'desc'        => esc_html__('Enter priority number for the Videos nav item. The lower the number is, the closer the nav item to the first is', '17jbh' ),
            'std'         => '10',
            'type'        => 'text',
            'section'     => 'buddypress');
    ...
    $settings['settings'][] = array(
            ...
            'label'       => esc_html__('Activity Items','17jbh'),
            'desc'        => esc_html__('Choose which actions to be included in the Activity. Once an user does the action, an activity log is recorded', '17jbh' ),
            'std'         => '',
            'type'        => 'checkbox',
            'section'     => 'buddypress',
            'choices'     => array(
                array(
                  'value'       => 'upload_video',
                  'label'       => esc_html__('Upload Video', '17jbh' )
                ),
                array(
                  'value'       => 'edit_video',
                  'label'       => esc_html__('Edit Video', '17jbh' )
                ),
                array(
                  'value'       => 'create_channel',
                  'label'       => esc_html__('Create Channel', '17jbh' )
                ),
                array(
                  'value'       => 'edit_channel',
                  'label'       => esc_html__('Edit Channel', '17jbh' )
                ),
                array(
                  'value'       => 'create_playlist',
                  'label'       => esc_html__('Create Playlist', '17jbh' )
                ),
                array(
                  'value'       => 'edit_playlist',
                  'label'       => esc_html__('Edit Playlist', '17jbh' )
                )
            ));
			
	$settings['settings'][] = array(
            'id'          => 'bp_notifications',
            'label'       => esc_html__('Show Notification Bubble', '17jbh'),
            'desc'        => esc_html__('Show Notification Bubble on Top Header', '17jbh' ),
            'std'         => 'off',
            'type'        => 'on-off',
            'section'     => 'buddypress'
          );
...
function videopro_setup_subnav() {
	...
	bp_core_new_subnav_item( array( 
		'name' => esc_html__( 'My Videos','17jbh'),
		'slug' => 'myvideos', 
    ...
    bp_core_new_subnav_item( array( 
		'name' => esc_html__( 'My Channels','17jbh'),
		'slug' => 'channels', 
		'parent_url' => trailingslashit( bp_displayed_user_domain() . 'videos' ) ,
		'parent_slug' => 'videos',
    ...
    bp_core_new_subnav_item( array( 
		'name' => esc_html__( 'My Playlists','17jbh'),
		'slug' => 'playlists', 
		'parent_url' => trailingslashit( bp_displayed_user_domain() . 'videos' ) ,
		'parent_slug' => 'videos',
    ...
    if(get_current_user_id() && get_current_user_id() == bp_displayed_user_id() && function_exists('osp_get')){
        ...
        if($enable_author_sub == 'on'){
            bp_core_new_subnav_item( array( 
                'name' => esc_html__( 'Subscribed Authors','17jbh'),
        ...
        if($enable_video_channels != false){
            bp_core_new_subnav_item( array( 
                'name' => esc_html__( 'Subscribed Channels','17jbh'),
    ...
    if(count($users) > 0){
        ...
        <div class="no-post">
            ...
            wp_reset_postdata();
                ...
                <a href="#" class="btn btn-default"><?php esc_html_e('Browse Authors','17jbh');?></a>
            <?php }?>
        </div>
    <?php }
}
...
function videopro_buddypress_playlists_tab(){
    ...
    <div class="cactus-listing-wrap">
        <div class="cactus-listing-config style-2"> <!--addClass: style-1 + (style-2 -> style-n)-->
            <div class="cactus-sub-wrap" id="bp_playlists">
                ...
                <section class="no-results not-found">
                    <div class="page-content">
                        <p>
                    <?php 
                    echo wp_kses_post(__('This author hasn\'t got any playlists yet','17jbh'));
                    ?>
                        </p>
                    </div>
                </section>
            ...
            <?php videopro_paging_nav_ajax('#bp_playlists','html/loop/content', esc_html__( 'Load More', '17jbh' ), $query); ?>
        </div>
    </div>
    <?php
}
...
function videopro_buddypress_channels_tab(){
    ...
    <div class="cactus-listing-wrap">
        <div class="cactus-listing-config style-2"> <!--addClass: style-1 + (style-2 -> style-n)-->
            <div class="cactus-sub-wrap" id="bp_channels">
                ...
                <section class="no-results not-found">
                    <div class="page-content">
                        <p>
                    <?php 
                    esc_html_e('This author hasn\'t got any channels yet','17jbh');
                    ?>
                        </p>
                    </div>
                </section>
            ...
            <?php videopro_paging_nav_ajax('#bp_channels','html/loop/content', esc_html__( 'Load More', '17jbh' ), $query); ?>
        </div>
    </div>
    <?php
}
...
function videopro_buddypress_videos_tab() {
    ...
    <div class="cactus-listing-wrap">
        <div class="cactus-listing-config style-2"> <!--addClass: style-1 + (style-2 -> style-n)-->
            <div class="cactus-sub-wrap" id="bp_videos">
                ...
                <section class="no-results not-found">
                    <div class="page-content">
                        <p>
                    <?php 
                    echo wp_kses_post(__('This author hasn\'t got any videos yet','17jbh'));
                    ?>
                        </p>
                    </div>
                </section>
            ...
            <?php videopro_paging_nav_ajax('#bp_videos','html/loop/content', esc_html__( 'Load More', '17jbh' ), $query); ?>
        </div>
    </div>
    <?php
}
    ...
    if(in_array('upload_video', $activity_items)){
        if(function_exists('bp_activity_add')){
            ...
            if($user_id && $post->post_status == 'publish'){
                ...
                $action = sprintf( esc_html__( '%s uploaded a new video', '17jbh' ), $userlink );
                if($is_uploaded_in_channel){
                    ...
                    $action = sprintf( esc_html__( '%s uploaded a new video in channel: %s ', '17jbh' ), $userlink, $channel_link );
                    ...
                    $action = sprintf( esc_html__( '%s uploaded a new video in playlist: %s', '17jbh' ), $userlink, $playlist_link );
                }
                ...
                $content = sprintf(wp_kses_post(__('<p>Check out this video:</p><br/> %s <br/> %s', '17jbh')), $thumbnail, $videolink);
    ...
    if(in_array('create_channel', $activity_items)){
        if(function_exists('bp_activity_add')){
            ...
            if($user_id){
                ...
                $content = sprintf(wp_kses_post('See channel: %s', '17jbh'), $channel_link);
                
                bp_activity_add( array(
                            'user_id' => $user_id,
                            'action' => apply_filters( 'videopro_bp_new_channel_activity_action', sprintf( esc_html__( '%s created a new channel', '17jbh' ), $userlink ), $user_id ),
    ...
    if(in_array('create_playlist', $activity_items)){
        if(function_exists('bp_activity_add')){
            ...
            if($user_id){
                ...
                $content = sprintf(wp_kses_post('See playlist: %s', '17jbh'), $playlist_link);
                
                bp_activity_add( array(
                            'user_id' => $user_id,
                            'action' => apply_filters( 'videopro_bp_new_playlist_activity_action', sprintf( esc_html__( '%s created a new playlist', '17jbh' ), $userlink ), $user_id ),
    ...
    if(in_array('edit_video', $activity_items)){
        if(function_exists('bp_activity_add')){
            ...
            if($user_id && !($error_upload)){
                ...
                $action = sprintf( esc_html__( '%s made some changes on a video', '17jbh' ), $userlink );
                ...
                $content = sprintf(wp_kses_post(__('<span class="activity-title">Video has been updated:</span> <span class="activity-thumbnail">%s</span> <br> <span class="activity-link">%s</span>', '17jbh')), $thumbnail, $videolink);
    ...
    if(in_array('edit_channel', $activity_items)){
        if(function_exists('bp_activity_add')){
            ...
            if($user_id){
                ...
                $content = sprintf(wp_kses_post('<p>Channel has been updated:</p> %s', '17jbh'), $channel_link);
                
                bp_activity_add( array(
                            'user_id' => $user_id,
                            'action' => apply_filters( 'videopro_bp_edit_channel_activity_action', sprintf( esc_html__( '%s made some changes on a channel', '17jbh' ), $userlink ), $user_id ),
    ...
    if(in_array('edit_playlist', $activity_items)){
        if(function_exists('bp_activity_add')){
            ...
            if($user_id){
                ...
                $content = sprintf(wp_kses_post('<p>Playlist has been updated:</p> %s', '17jbh'), $playlist_link);
                
                bp_activity_add( array(
                            'user_id' => $user_id,
                            'action' => apply_filters( 'videopro_bp_edit_playlist_activity_action', sprintf( esc_html__( '%s made some changes on a playlist', '17jbh' ), $userlink ), $user_id ),
 ...
 */ 
 function videopro_buddypress_add_custom_components( $component_names = array() ) {
	...
	array_push( $component_names, '17jbh' );
...
function videopro_buddypress_format_notifications( $description, $notification) {
	...
	if ( 'upload_video_channel' === $action || 'upload_video_playlist' === $action || 'upload_new_post' === $action ) {
	
		$comment = get_comment( $item_id );
	
		$custom_title = esc_html__('New video uploaded', '17jbh');
        ...
        if ('upload_video_channel' === $action) {
            $custom_text = sprintf(esc_html__('New video uploaded in channel: %s', '17jbh'), $name);
        } elseif ('upload_new_post' === $action) {
            if ($post_format == 'post-format-video' || $post_format == 'video') {
                $custom_text = sprintf(esc_html__('New video uploaded by ' . $author_name, '17jbh'), $name);
            } else {
                $custom_title = esc_html__('New post created', '17jbh');
                $custom_text = sprintf(esc_html__('New post created by ' . $author_name, '17jbh'), $name);
            }
        } else {
            $custom_text = sprintf(esc_html__('New video uploaded in playlist: %s', '17jbh'), $name);
        }
