...
function videopro_acplugins($plugins) {
     ...
     */
    $config = array(
        'domain'            => '17jbh',           // Text domain - likely want to be the same as your theme.
        ...
        'strings'           => array(
            'page_title'                                => esc_html__( 'Install Required &amp; Recommended Plugins', '17jbh' ),
            'menu_title'                                => esc_html__( 'Install Plugins', '17jbh' ),
            'installing'                                => esc_html__( 'Installing Plugin: %s', '17jbh' ), // %1$s = plugin name
            'oops'                                      => esc_html__( 'Something went wrong with the plugin API.', '17jbh' ),
            'notice_can_install_required'               => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', '17jbh' ), // %1$s = plugin name(s)
            'notice_can_install_recommended'            => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', '17jbh' ), // %1$s = plugin name(s)
            'notice_cannot_install'                     => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', '17jbh' ), // %1$s = plugin name(s)
            'notice_can_activate_required'              => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', '17jbh' ), // %1$s = plugin name(s)
            'notice_can_activate_recommended'           => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', '17jbh' ), // %1$s = plugin name(s)
            'notice_cannot_activate'                    => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', '17jbh' ), // %1$s = plugin name(s)
            'notice_ask_to_update'                      => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', '17jbh' ), // %1$s = plugin name(s)
            'notice_cannot_update'                      => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', '17jbh' ), // %1$s = plugin name(s)
            'install_link'                              => _n_noop( 'Begin installing plugin', 'Begin installing plugins', '17jbh' ),
            'activate_link'                             => _n_noop( 'Activate installed plugin', 'Activate installed plugins', '17jbh' ),
            'return'                                    => esc_html__( 'Return to Required Plugins Installer', '17jbh' ),
            'plugin_activated'                          => esc_html__( 'Plugin activated successfully.', '17jbh' ),
            'complete'                                  => esc_html__( 'All plugins installed and activated successfully. %s', '17jbh' ) // %1$s = dashboard link
        )
    );
...
function videopro_icon_class_widget_add_custom_fields() {
	...
	echo "<p><label for='".$id_disp."-icon_class'>".esc_html__('Title Icon Class ', '17jbh').": <input class='widefat' type='text' name='".$id_disp."-icon_class' id='".$id_disp."-icon_class' value='".$value."' placeholder='".esc_html__('Enter Font Awesome Class for the icon', '17jbh')."' /></label></p>";
}
...
function videopro_sub_label_widget_add_custom_fields() {
	...
	// output our extra widget logic field
	echo "<p><label for='".$id_disp."-sub_label'>".esc_html__('Badge Text', '17jbh').": <input class='widefat' type='text' name='".$id_disp."-sub_label' id='".$id_disp."-sub_label' value='".$value."' placeholder='".esc_html__('Enter badge text', '17jbh')."'/></label></p>";
}
...
function videopro_color_widget_add_custom_fields() {
	...
	// output our extra widget logic field
	echo "<p><label for='".$id_disp."-ct_color'>".esc_html__('Badge Text Color ', '17jbh').": 
	<span style='font-style: italic; display:block;'>".esc_html__('choose text color for the badge', '17jbh')."</span>
...
function videopro_bgcolor_widget_add_custom_fields() {
	...
	echo "<p><label for='".$id_disp."-ct_bgcolor'>".esc_html__('Badge Background Color', '17jbh').": 
	<span style='font-style: italic; display:block;'>".esc_html__('choose background color for the Badge ', '17jbh')."</span>
...
{   
    ...
    // output our extra widget logic field
    echo "
	<p id='style-".$id_disp."'><label for='".$id_disp."-cactusthemes_style'>".__('Widget Styles', '17jbh').": 
	<select name='".$id_disp."-cactusthemes_style' id='".$id_disp."-cactusthemes_style'>
	  <option value='' ".($value==''?'selected="selected"':'').">".esc_html__('Simple','17jbh')."</option>
	  <option value='style-2' ".($value=='style-2'?'selected="selected"':'').">".esc_html__('Simple border top','17jbh')."</option>
	  <option value='style-3' ".($value=='style-3'?'selected="selected"':'').">".esc_html__('Boxed','17jbh')."</option>
	  <option value='style-4' ".($value=='style-4'?'selected="selected"':'').">".esc_html__('Bordered','17jbh')."</option>
	  <option value='style-5' ".($value=='style-5'?'selected="selected"':'').">".esc_html__('Simple No Border','17jbh')."</option>
	</select>
	</label></p>";
}
/**
...
{   
    ...
    // output our extra widget logic field
    echo "
	<p class='uni-footer-width' id='uni-".$id_disp."'><label for='".$id_disp."-cactusthemes_width'>".__('Widget width', '17jbh').": 
...
function videopro_widget_add_custom_fields() {
	...
	// output our extra widget logic field
	echo "<p><label for='".$id_disp."-cactusthemes'>".esc_html__('Widget Custom Variation ', '17jbh').": 
	<span style='font-style: italic; display:block;'>".esc_html__('enter your own custom css class', '17jbh')."</span>
