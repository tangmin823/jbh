<form role="search" class="advance-search-form" onsubmit="if(jQuery('.ss',jQuery(this)).val() == '<?php echo esc_attr($default_text);?>' || jQuery('.ss',jQuery(this)).val() == '') return false;" method="get" id="form-ss" action="<?php echo esc_url(home_url( '/' )); ?>">
	<div>		
		...
		<span class="lookin">
		<label for="cat"><?php echo esc_html__('Look in: ','17jbh');?><select id="s-cat" name="cat">
			<option value=""><?php echo " ".esc_html__('All categories','17jbh')." ";?></option>
			<?php
		...
		<?php 
				...
				<span class="orderby">
					<label><?php echo esc_html__('Order by','17jbh');?>
					<select name="orderby">
						<option value="post_title"> <?php echo esc_html__('Alphabetic','17jbh');?> </option>
						<option value="post_date"> <?php echo esc_html__('Updated date','17jbh');?> </option>
					</select>
					</label>
				</span>
				<span class="order-cleaner"><!-- --></span>
				<span class="order">
					<label><?php echo esc_html__('Order','17jbh');?>
					<select name="order">
						<option value="asc"> <?php echo esc_html__('Asc','17jbh');?> </option>
						<option value="desc"> <?php echo esc_html__('Desc','17jbh');?> </option>
					</select>
					</label>
				</span>
				<?php if($show_tag_filter){
					...
					<span class="search_tags">
						<a href="javascript:void(0)" <?php if(isset($_GET['tag']) && $_GET['tag'] == '') echo 'class="filtered"';?> onclick="jQuery('#ss_tag_name').val('');jQuery('#form-ss').submit()"><?php echo esc_html__('Any tag','17jbh');?></a>
					<?php
						foreach($tags as $tag){
					?>
