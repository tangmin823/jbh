...
class VideoPro_Actor_Listing extends WP_Widget {	
	...
	function __construct() {
    	$widget_ops = array(
			...
			'description' => esc_html__('VideoPro - Actor Listing','17jbh')
		);
    	parent::__construct('actor-listing', esc_html__('VideoPro - Actor Listing','17jbh'), $widget_ops);
	}
			...
			while($the_query->have_posts()): $the_query->the_post();
				...
				<div class="channel-subscribe">';
					...
					<div class="channel-content">
                        ...
                        if($it > 0){
                            $html .='
                            <span class="tt-number cactus-info font-size-1">'.sprintf(__('%d videos', '17jbh'), $it).'</span>';
                        }
							$html .='
					</div>
	...
	function form( $instance ) {
        ...
        <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
      	<!-- /**/-->
        <p>
          <label for="<?php echo $this->get_field_id('ids'); ?>"><?php esc_html_e('IDs (Specify actor\'s IDs to show):','17jbh'); ?></label> 
          <input class="widefat" id="<?php echo $this->get_field_id('ids'); ?>" name="<?php echo $this->get_field_name('ids'); ?>" type="text" value="<?php echo $ids; ?>" />
        </p>
        <p>
          <label for="<?php echo $this->get_field_id('tags'); ?>"><?php esc_html_e('Tags:','17jbh'); ?></label> 
