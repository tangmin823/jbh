...
function videopro_report_post_type() {
	$args = array(
		'labels' => array(
			'name' => esc_html__( 'Reports', '17jbh' ),
			'singular_name' => esc_html__( 'Report', '17jbh' )
		),
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title h4" id="myModalLabel"><?php esc_html_e('Report This','17jbh'); ?></h4>
              </div>
              <div class="modal-body">
                    ...
                    if ($form_id_cf7 != '') {
                        ...
                        $value = apply_filters( 'videopro_report_spam_video_form', esc_html__('Please specify an ID for the Contact Form in Video Settings > Video Post > Spam Flag-Contact Form 7 ID or Spam Flag-Gravity Form ID','17jbh') );
                        echo $value;
                    }
					?>
              </div>
...
if(!function_exists('videopro_user_submit_video_button_html')) {
	function videopro_user_submit_video_button_html( $context = '') {
		if(osp_get('ct_video_settings','user_submit') == 1) {
			...
			if($text_bt_submit == '') {
				$text_bt_submit = esc_html__('Submit Video','17jbh');
			}
            ...
            <div class="video-toolbar-content">
                <div class="toolbar-right">
					...
					if(!empty($p)){
						...
						?>
						<a href="<?php  echo esc_url($pv_link);?>" class="btn btn-default video-tb font-size-1 cactus-new prev-video"><i class="fa fa-chevron-left"></i><span><?php echo esc_html__( '上一个视频', '17jbh' )?></span></a>
					<?php
					}
					if(!empty($n)){
						...
						?>
                    	<a href="<?php echo esc_url($nv_link); ?>" class="btn btn-default video-tb font-size-1 cactus-old next-video"><span><?php echo esc_html__( '下一个视频', '17jbh' )?></span><i class="fa fa-chevron-right"></i></a>
					<?php
                    }
					...
					if($show_more != 'off' && !empty($ct_query_more)){?>
                    	<a href="#" class="btn btn-default video-tb font-size-1 open-carousel-post-list"><span><?php esc_html_e('更多视频','17jbh');?></span><i class="fa fa-caret-down"></i></a>
                    <?php }?>
                </div>
            </div>
			<?php
                            ...
                            <div class="cactus-listing-wrap">
                                <div class="cactus-listing-config style-2"> <!--addClass: style-1 + (style-2 -> style-n)-->
                                    <div class="cactus-sub-wrap">
                                        ...
                                        ?>
                                        <!--item listing-->
                                        <article class="cactus-post-item <?php echo $active;?>">
...
if(!function_exists('videopro_singlevideo_right_meta_html')) {
	function videopro_singlevideo_right_meta_html($html, $post_format) {
		...
		<div class="right">
			...
			if($download_link){?>
				<a href="<?php echo $download_link;?>" target="_blank" class="btn btn-default ct-gradient bt-action metadata-font font-size-1"><span><?php echo $download_button ? $download_button : wp_kses(__('<i class="fa fa-cloud-download"></i> Download Video', '17jbh'),array('i'=>array('class'=>array())));?></span></a>
			<?php
			}
...
function videopro_auto_next_video_html($auto_next_html){
    ...
    if($single_video){
        ...
        if($enable_light_switch == 1){
            $html .= '<a href="#" id="videopro_light_on" ' . ($has_auto_next_button ? '' : 'class="no-margin"') . '>' . esc_html__('LIGHT','17jbh') . '<i class="fas fa-lightbulb"></i></a>';
        }

        if($has_auto_next_button){
            $html .= '<div class="auto-text">'.esc_html__('AUTO NEXT','17jbh').'</div>
		...
		if($enable_archives_lightbox == '1'){?>
			<div class="ct-icon-video lightbox_item<?php if(isset($class) && $class!=''){ echo ' '.esc_attr($class);}?>" data-source="" data-type="iframe-video" data-caption="<?php the_title_attribute(); ?>" data-id="<?php echo esc_attr($id);?>">
				<?php
					...
					if( osp_get('ct_video_settings','single_player_video') == 'flowplayer' && function_exists('flowplayer_content_handle') ){
...
function videopro_loop_item_thumbnail_filter($html, $id, $img_size, $post_format, $video_data, $context = ''){
		...
		<div class="picture-content <?php echo $screenshot_preview ? 'screenshots-preview-inline' : '';?>" data-post-id="<?php echo $id; ?>">
                    ...
                    <a href="#" title="<?php echo esc_html__('Remove from Watch Later', '17jbh');?>" class="btn btn-default video-tb icon-only font-size-1 btn-watch-later" data-id="<?php echo $id;?>" data-action="remove"><i class="fa fa-remove"></i></a>
                    <?php
                        } else {
                        ?>
                    <a href="#" title="<?php echo esc_html__('Watch Later', '17jbh');?>" class="btn btn-default video-tb icon-only font-size-1 btn-watch-later" data-id="<?php echo $id;?>"><i class="fas fa-clock"></i></a>
                    <?php
                        }
                    }
                    ?>
...
?>
                <a href="#" title="<?php echo esc_html__('Watch Later', '17jbh');?>" class="btn btn-default video-tb icon-only font-size-1 btn-watch-later" data-id="<?php echo $post_id;?>"><i class="fas fa-clock"></i></a>
<?php
        }
    }
}
...
function videopro_loop_item_show_status(){
    ...
    if($post->post_status != 'publish'){
        echo '<div class="post-status">' . esc_html__('Status: ', '17jbh') . ' ' . $post->post_status . '</div>';
    }
}
...
function videopro_add_verified_icon( $id, $type = '' ){
    ...
    if($is_verified){
        echo '<span class="verified" title="' . esc_html__('Verified', '17jbh') . '"><i class="fa fa-check"></i></span>';
    }
}
...
function videopro_userprofile_show_extra_fields( $user ) {
    ...
    if ( !current_user_can( 'manage_options' ) ){
        ...
        ?>
        <h3><?php esc_html_e('Is Verified Author', '17jbh');?></h3>
        <label><?php esc_html_e('Verified', '17jbh');?>: <input type="checkbox" name="is_verified" <?php echo $is_verified ? 'checked="checked"' : '';?>/></label>
        <p><?php esc_html_e('Check to display a Verified Icon to Author Name', '17jbh');?></p>
<?php
}
