...
$text_translate_video_st = esc_html__('General','17jbh').esc_html__('Player for Video File','17jbh').esc_html__('JWPlayer','17jbh').esc_html__('FlowPlayer','17jbh').esc_html__('VideoJS - HTML5 Video Player','17jbh').esc_html__('WordPress Native Player: MediaElement','17jbh').esc_html__('Force Using VideoJS for external videos','17jbh').esc_html__('Yes','17jbh').esc_html__('No','17jbh').esc_html__('Auto Play Video','17jbh').esc_html__('Yes','17jbh').esc_html__('No','17jbh').esc_html__('Auto Load Next Video','17jbh').esc_html__('Yes','17jbh').esc_html__('No','17jbh').esc_html__('Auto Load Next Video after','17jbh').esc_html__('Enter number in seconds. Ex: 5 (seconds)','17jbh').esc_html__('Choose Next Video for Auto Load','17jbh').esc_html__('Newer Video','17jbh').esc_html__('Older Video','17jbh').esc_html__('Youtube Settings','17jbh').esc_html__('Force Using JWPlayer','17jbh').esc_html__('No','17jbh').esc_html__('Yes','17jbh').esc_html__('Related videos','17jbh').esc_html__('Display related videos at the end of the video','17jbh').esc_html__('Hide','17jbh').esc_html__('Show','17jbh').esc_html__('Use HTML5 player','17jbh').esc_html__('Use HTML5 player to play YouTube videos','17jbh').esc_html__('No','17jbh').esc_html__('Yes','17jbh').esc_html__('Show Video Info on player','17jbh').esc_html__('Show','17jbh').esc_html__('Hide','17jbh').esc_html__('Remove annotations on video','17jbh').esc_html__('Yes','17jbh').esc_html__('No','17jbh').esc_html__('Force using Embed Code','17jbh').esc_html__('No','17jbh').esc_html__('Yes','17jbh').esc_html__('Allow Full Screen','17jbh').esc_html__('Yes','17jbh').esc_html__('No','17jbh').esc_html__('Allow Networking','17jbh').esc_html__('Allow Interactive Videos','17jbh').esc_html__('Disable','17jbh').esc_html__('Enable','17jbh').esc_html__('Playlist settings','17jbh').esc_html__('Playlist slug','17jbh').esc_html__('Change single playlist slug. Remember to save the permalink settings again in Settings > Permalinks','17jbh').esc_html__('Playlists Listing page','17jbh').esc_html__('Assign Playlists Listing page to a page. Remember to save the permalink settings again in Settings > Permalinks','17jbh');
...
class Cactus_video {
	...
	function register_page_templates(){
        ...
        $page_templ = array('cactus-video/includes/page-templates/watch-later.php' => esc_html__('Watch Later', '17jbh'));

		if($enable_video_playlist != false){
            $page_templ = array_merge($page_templ, array(
				'cactus-video/includes/page-templates/playlist-listing.php' => esc_html__('Playlist Listing Page', '17jbh')
			));
        }

        if($enable_video_channels != false){
			$page_templ = array_merge($page_templ, array(
				'cactus-video/includes/page-templates/subscribed-channels.php' => esc_html__('Subscribed Channels Page', '17jbh'),
				'cactus-video/includes/page-templates/channel-listing.php' => esc_html__('Channel Listing Page', '17jbh')
			));
		}

        if($enable_video_series == 1){
            $page_templ = array_merge($page_templ, array(
                    'cactus-video/includes/page-templates/video-series.php' => esc_html__('Video Series Listing Page', '17jbh'),
                ));
        }

        if($enable_author_sub == 'on'){
            $page_templ = array_merge($page_templ, array(
                    'cactus-video/includes/page-templates/subscribed-authors.php' => esc_html__('Subscribed Authors Page', '17jbh'),
                ));
        }
	...
	function ct_video_scripts_styles() {
        ...
        $js_params['lang']['confirm_delete_playlist'] = esc_html__('You are about to delete a playlist. Are you sure?', '17jbh');
        $js_params['lang']['confirm_delete_channel'] = esc_html__('You are about to delete a channel. Are you sure?', '17jbh');
        $js_params['lang']['please_choose_category'] = esc_html__('Please choose a category', '17jbh');
        $js_params['lang']['please_choose_channel'] = esc_html__('Please choose a channel', '17jbh');
        $js_params['lang']['please_choose_playlist'] = esc_html__('Please choose a playlist', '17jbh');
	...
	function register_configuration(){
		...
		$ct_video_settings = new Options_Page('ct_video_settings', array('option_file'=>dirname(__FILE__) . '/options.xml','menu_title' => esc_html__('Video Extensions','17jbh'),'menu_position'=>null), array('page_title'=>esc_html__('Video Extensions - Settings','17jbh'),'submit_text'=>esc_html__('Save','17jbh')));

		global $ct_channel_settings;
		$ct_channel_settings = new Options_Page('ct_channel_settings', array('option_file' => dirname(__FILE__) . '/options-channel.xml','menu_title'=>esc_html__('Video Channels','17jbh'),'menu_position'=>null,'parent_menu' => 'ct_video_settings'), array('page_title' => esc_html__('Video Channels','17jbh'),'submit_text'=>esc_html__('Save','17jbh')));

		global $ct_playlist_settings;
		$ct_playlist_settings = new Options_Page('ct_playlist_settings', array('option_file' => dirname(__FILE__) . '/options-playlist.xml','menu_title'=>esc_html__('Video Playlist','17jbh'),'menu_position'=>null,'parent_menu' => 'ct_video_settings'), array('page_title' => esc_html__('Video Playlist','17jbh'),'submit_text'=>esc_html__('Save','17jbh')));
	}
	...
	function init(){
        load_plugin_textdomain( '17jbh', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
    ...
    function custom_title($title){
        if($this->is_edit_post_template()){
            $title['title'] = esc_html__('Edit Post', '17jbh');
        }
    ...
    function show_uploaded_message(){
        $html = '<div class="fading_message">' . esc_html__('Image has been uploaded!', '17jbh') . '</div>';
    ...
    function show_saved_message(){
        $html = '<div class="fading_message">' . esc_html__('Changes have been saved!', '17jbh') . '</div>';
    ...
    function show_error_message(){
        ...
        if(isset($_GET['save_error'])){
            ...
            if($error == -1){
                $message = esc_html__('You do not have enough permissions to do this!!!', '17jbh');
            }
        }
    ...
    function show_error_upload_message(){
        ...
        switch($error){
            default:
                // do something;
                $msg = esc_html__('Upload failed!', '17jbh');
                break;
        }
    ...
    function filter_option_tree_settings($settings){
        $settings['settings'][] = array(
            ...
            'desc'        => esc_html__('This Ads appears on the Left Side of Video Player (full-width layout only). Enter Google AdSense Ad Slot ID here. If left empty, "Video Player Side-Ads Left - Custom Code" will be used.', '17jbh' ),
            'std'         => '',
            'type'        => 'text',
            'section'     => 'advertising'
          );
        ...
        $settings['settings'][] = array(
            ...
            'desc'        => esc_html__('Custom Code for Video Player Side-Ads Left', '17jbh' ),
            'std'         => '',
        ...
        $settings['settings'][] = array(
            ...
            'desc'        => esc_html__('This Ads appears on the Right Side of Video Player (full-width layout only). If left empty, "Single Post Ads 2 - Custom Code" will be used.', '17jbh' ),
            'std'         => '',
            'type'        => 'text',
            'section'     => 'advertising'
          );
        ...
        $settings['settings'][] = array(
            ...
            'desc'        => esc_html__('Custom Code for Video Player Side-Ads Right', '17jbh' ),
            'std'         => '',
            'type'        => 'textarea-simple',
            'section'     => 'advertising'
          );
          ...
          $settings['settings'][] = array(
            ...
            'label'       => esc_html__( 'Enable Membership Features', '17jbh' ),
            'desc'        => esc_html__( 'Allow Members to create and upload videos', '17jbh' ),
            'std'         => 'off',
            'type'        => 'on-off',
            'section'     => 'membership',
            'operator'    => 'and'
          ...
          $settings['settings'][] = array(
            ...
            'label'       => esc_html__( 'Agreement Text', '17jbh' ),
            'desc'        => esc_html__( 'Change "I agree to the Page Term" text in the Create Channel Form. A hyperlink can be used here', '17jbh' ),
            'std'         => '',
            'type'        => 'text',
            'section'     => 'membership',
          ...
          $settings['settings'][] = array(
            ...
            'label'       => esc_html__( 'Allow Edit Video Categories and Tags', '17jbh' ),
            'desc'        => esc_html__( 'After users upload video, allow them to change categories and tags of the video', '17jbh' ),
            'std'         => 'on',
            'type'        => 'on-off',
            'section'     => 'membership',
            'condition'   => 'membership_allow_creating_channel:is(on)'
    ...
    function add_watch_later(){
        ...
        if($post_id && $user_id){
            ...
            if(!in_array($post_id, $posts_list)){
                ...
                $result['status'] = 1;
                $result['message'] = esc_html__('Added to Watch Later', '17jbh');
                ...
                if($todo == 'remove'){
                    ...
                    $result['status'] = -1;
                    $result['message'] = esc_html__('Removed from Watch Later', '17jbh');
                }
            }
        } else {
            $result['status'] = 0;
            $sign_in_url = wp_login_url($current_url);
            $result['message'] = sprintf(wp_kses(__('<a href="%s">Sign in</a> to add this to Watch Later', '17jbh'), array('a' => array('href' => array()))), $sign_in_url);
        }
	...
	function add_toolbar_link( $wp_admin_bar ) {
	    if (current_user_can('manage_options')) {
            $args = array(
                ...
                'title' => esc_html__('Video Extension','17jbh'),
    ...
    function ajax_remove_post(){
        ...
        if($user_id && $user_id == $author_id && in_array($post_type, array('post','ct_playlist', 'ct_channel'))){
            ...
            switch($post_type){
                case 'post':
                    $result['message'] = esc_html__('Video is trashed. If you accidently deleted this video, please contact site administrator','17jbh');
                    break;
                case 'ct_channel':
                    $result['message'] = esc_html__('Channel is trashed. If you accidently deleted this channel, please contact site administrator','17jbh');
                    break;
                case 'ct_playlist':
                    $result['message'] = esc_html__('Playlist is trashed. If you accidently deleted this playlist, please contact site administrator','17jbh');
                    break;
            }
	...
	function register_post_type_metadata(array $meta_boxes){
		...
		$video_fields = array(
				array( 'id' => 'tm_video_url', 'name' => esc_html__( 'Video URL','17jbh'), 'type' => 'text','desc' => wp_kses(__( 'Paste the url from popular video sites like YouTube or Vimeo. For example: <br/><code>http://www.youtube.com/watch?v=nTDNLUzjkpg</code><br/>or<br/><code>http://vimeo.com/23079092</code>','17jbh'),array('br'=>array()),array('code'=>array())),  'repeatable' => false, 'multiple' => false ),
				array( 'id' => 'tm_video_file', 'name' => esc_html__('Video File','17jbh'), 'type' => 'textarea', 'desc' => wp_kses(__( 'Paste your video file url to here. Supported Video Formats: mp4, m4v, webmv, webm, ogv and flv.<br/><b>About Cross-platform and Cross-browser Support</b><br/>If you want your video works in all platforms and browsers(HTML5 and Flash), you should provide various video formats for same video, if the video files are ready, enter one url per line.<br/> For Example:<br/> <code>http://yousite.com/sample-video.m4v</code><br/><code>http://yousite.com/sample-video.ogv</code><br/> <b>Recommended Format Solution:</b> webmv + m4v + ogv. ','17jbh'),array('br'=>array()),array('b'=>array()),array('code'=>array())),  'repeatable' => false, 'multiple' => false ),
                array('id' => 'videopro_video_file', 'name' => esc_html__('Upload/Choose Video File','17jbh'), 'type' => 'file', 'file_type' => 'video', 'repeatable' => true),
				array( 'id' => 'tm_video_code', 'name' => esc_html__('Video Embeded Code'), 'type' => 'textarea', 'desc' => wp_kses(__( 'Paste the raw video code to here, such as <code>&lt;</code><code>object</code><code>&gt;</code>,<code>&lt;</code><code>embed</code><code>&gt;</code> or <code>&lt;</code><code>iframe</code><code>&gt;</code> code.','17jbh'),array('br'=>array()),array('b'=>array()),array('code'=>array())),  'repeatable' => false, 'multiple' => false ),
				array( 'id' => 'video_duration', 'name' => esc_html__('Duration'), 'desc' => esc_html__('Human-read time value, ex. mm:ss. Leave empty to fetch data again or enter your own value here', '17jbh'), 'type' => 'text',  'repeatable' => false, 'multiple' => false ),
				array( 'id' => 'video_download_url', 'name' => esc_html__('Download Link'), 'desc' => esc_html__('If this video can be downloaded, provide the download link here. In fact, you can add any link here', '17jbh'), 'type' => 'text' ),
				array( 'id' => 'video_download_button', 'name' => esc_html__('Download Button Text'), 'desc' => esc_html__('Change the Download Button text', '17jbh'), 'type' => 'text' ),
                array( 'id' => 'external_url', 'name' => esc_html__('External Link'), 'desc' => esc_html__('If this video has external link then instead of playing video, users will be redirected to this URL when they click on thumbnail image', '17jbh'), 'type' => 'text' ),
                array(
                        'id' => 'has_single_page',
                        'name' => esc_html__('Has Single Page'),
                        'desc' => esc_html__('Choose to disable single page for this video. It means that users will be redirected to External Link when clicking on thumbnail image of this video in archives page', '17jbh'),
                        'type' => 'select',
                        'options' => array('' => esc_html__('Yes', '17jbh'),
                                            'no' => esc_html__('No (use External Link)', '17jbh'))
                    ),
                array(
                        'id' => 'video_player',
                        'name' => esc_html__('Video Header'),
                        'desc' => esc_html__('Choose to display Thumbnail Image in Single Post, or display Video Player', '17jbh'),
                        'type' => 'select',
                        'options' => array('' => esc_html__('Default (use setting in Video Extension setting page)', '17jbh'),
                                            '2' => esc_html__('Thumbnail Image', '17jbh'),
                                            '1'    => esc_html__('Video Player', '17jbh'))
                    ),
				array(
                        'id' => 'use_jwplayer_settings',
                        'name' => esc_html__('Use JW Player Settings'),
                        'desc' => esc_html__('If you use JW Player plugin for your video, and you want to let JW Player totally control the player, turn this on. You can find the setting box for JW Player underneath', '17jbh'),
                        'type' => 'select',
                        'options' => array( 0 => esc_html__('Default (use setting in Video Extension setting page)', '17jbh'),
                                            2 => esc_html__('No', '17jbh'),
                                            1    => esc_html__('Yes', '17jbh'))
                    )
			);

		$meta_boxes[] = array(
			'title' => esc_html__('Video settings','17jbh'),
			'pages' => 'post',
			'fields' => $video_fields,
			'priority' => 'high'
		);
		...
		$playlogic_fields = array(
				array( 'id' => 'player_logic', 'name' => esc_html__( 'Player logic','17jbh'), 'type' => 'text','desc' => wp_kses(__( 'Enter shortcode (ex: [my_shortcode][player][/my_shortcode], <strong>[player]</strong> is required)<br>or conditional function - any function which returns true or false value (ex: <b>is_user_logged_in()</b> ) - to control video player visiblitily','17jbh'),array('br'=>array()),array('strong'=>array()),array('code'=>array())) ,  'repeatable' => false, 'multiple' => false ),
				array( 'id' => 'player_logic_alt', 'name' => esc_html__('Alternative Content','17jbh'), 'type' => 'text', 'desc' => esc_html__( 'Content to display when Condition is false (Not work with Shortcodes)','17jbh') ,  'repeatable' => false, 'multiple' => false ),
			);
		$meta_boxes[] = array(
			'title' => esc_html__('Player Logic','17jbh'),
			'pages' => 'post',
        ...
        $custom_player_shortcode_fields = array(
            array( 'id' => 'custom_player_shortcode', 'name' => esc_html__( 'Custom Player Shortcode','17jbh'), 'type' => 'text','desc' => wp_kses(__( 'Enter your custom player shortcode (from your themes, plugins,...) if you want to run your own shortcode player instead of normal video information above.'),array('br'=>array()),array('strong'=>array()),array('code'=>array())) ,  'repeatable' => false, 'multiple' => false ),
        );
        $meta_boxes[] = array(
            'title' => esc_html__('Custom Player Shortcode','17jbh'),
            'pages' => 'post',
		...
		$auto_fetch_data = array(
				array( 'id' => 'fetch_info',  'name' => esc_html__('Do Not Fetch','17jbh'), 'type' => 'checkbox','multiple' => false,  'desc' => esc_html__('Check bellow checkbox if you do not want to auto-fetch video data after save/edit. To chose which fields to fetch, go to Video Extensions setting page','17jbh') , 'repeatable' => false ),
		);
		$meta_boxes[] = array(
			'title' => esc_html__('Videos Auto Fetch Data','17jbh'),
			'pages' => 'post',
...
function fvp_player_iframe(){
	...
	if( is_singular() && isset( $_GET['fvp-player-embed'] ) && is_plugin_active( 'fv-wordpress-flowplayer/flowplayer.php' ) ){
