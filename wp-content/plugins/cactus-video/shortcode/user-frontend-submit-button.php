			...
			jQuery("form.wpcf7-form").submit(function (e) {
				...
				if(submit_tags.length > <?php echo $limit_tags ?>){
					if(jQuery('.limit-tag-alert').length==0){
						jQuery('.wpcf7-form-control-wrap.tag').append('<span role="alert" class="wpcf7-not-valid-tip limit-tag-alert"><?php echo sprintf(esc_html__('Please enter less than or equal to %s tags', '17jbh'), $limit_tags); ?>.</span>');
					}
					return false;
				}else{
					return true;
