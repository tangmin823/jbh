...
/** 1040 is GTMatrix screen size **/
$source_sizes = '(max-width: 480px) 100vw, (max-width: 767px) 340px, (max-width: 1040px) 192px, (min-width: 1280px) and (max-width: 1600px) 320px, '.$img_size[0].'px';
...
if($screenshots_preview){
	$thumb_html = videopro_get_post_screenshots_html($id, $img_size, $source_sizes);
                    ...
                    <div class="picture-content <?php echo $screenshots_preview ? 'screenshots-preview-inline' : '';?>">
                        <a href="<?php echo esc_url($link_post); ?>" target="<?php echo apply_filters('videopro_loop_item_url_target', '_self', $id);?>" title="<?php the_title_attribute(); ?>">
							...
							if($screenshots_preview){
								...
								echo videopro_thumbnail($img_size, $id, $source_sizes);
							}
