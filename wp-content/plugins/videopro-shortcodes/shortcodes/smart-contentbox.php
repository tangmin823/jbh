...
function videopro_smart_contentbox_filters($atts, $content = null) {
    ...
    <div class="<?php echo esc_attr($clss);?>">
    	...
    	<span class="cur-item" data-filter="0" ><?php if($title!=''){ echo esc_attr($title);}else{ esc_html_e('Videos','17jbh');}?></span>
...
function videopro_smart_contentbox($atts, $content = null) {
    ...
    } elseif (isset($atts['condition_other_posttype'])) {
        $condition = $atts['condition_other_posttype'];
    } else {
        $condition = 'latest';
    }
	...
	$args = smartcontentbox_query($items_per_page,$condition,$order,$cats,$tags,$ids,$page,$offset,$post_format, $time_range,$posttype);
		...
		vc_map( array(
			...
			"params"	=> array(
                ...
                array(
                    ...
                    "type" => "dropdown",
                    "heading" => esc_html__("Condition", "videopro"),
                    "param_name" => "condition_other_posttype",
                    "value" => array(
                        esc_html__("Latest - order by published date","videopro")=>'latest',
                        esc_html__("View - order by most viewed posts","videopro")=>'view',
                        esc_html__("Title - order by title alphabetically", "videopro") => "title",
                    ...
                    "dependency" => array(
                        "element" => "posttype",
                        "value" => array( "ct_actor", "ct_channel", "ct_playlist" ),
                    )
                ),
                array(
					"admin_label" => false,
				...
				array(
					...
					'group' => esc_html__('Appearance', '17jbh')
				),
				array(
					"type" => "dropdown",
                ...
                array(
					...
					esc_html__("Yes", "videopro") => "1",
					esc_html__("No", "videopro") => "0" ),
					"description" => esc_html__('Show post View Count number', '17jbh'),
					'group' => esc_html__('Appearance', '17jbh')
				),
				array(
					"type" => "dropdown",
				...
				array(
					...
					esc_html__("Yes", "videopro") => "1",
					esc_html__("No", "videopro") => "0" ),
					"description" => esc_html__('Show post rating value, require Cactus-Rating plugin installed', '17jbh'),
					'group' => esc_html__('Appearance', '17jbh')
				),
				array(
					"type" => "dropdown",
				...
				array(
					...
					esc_html__("Yes", "videopro") => "1",
					esc_html__("No", "videopro") => "0" ),
					"description" => esc_html__('Show a border at bottom of the box', '17jbh')
				),
				array(
					"type" => "dropdown",
				...
				array(
					...
					esc_html__("No", "videopro") => "0",
					esc_html__("Yes", "videopro") => "1" ),
					"description" => esc_html__('Enable Video Player for video items. Applied for layout 7, 8, 9, 10, 11', '17jbh'),
                    "dependency" => array(
                        "element" => "posttype",
                        "value" => array( "post" ),
                    )
				...
				array(
					...
					esc_html__("Yes", "videopro") => "1",
					esc_html__("No", "videopro") => "0" ),
					"description" => esc_html__('Enable Video Screenshots preview when hovering on items', '17jbh'),
                    "dependency" => array(
                        "element" => "posttype",
                        "value" => array( "post" ),
                    )
